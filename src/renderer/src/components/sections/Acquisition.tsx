...
export const Acquisition = (): ReactNode => {
      ...
      >
        <form
          onSubmit={(event) => {
            ...
            if (!p) {
              S = Number(s)
              pValue = ((A - S) / tValue / A) * 100
              result = 'p = ' + pValue.toFixed(2) + ' %'
            } else {
              pValue = Number(p)
              S = A - tValue * (A * (pValue / 100))
            ...
            const Dn = A * i
            const table = []
            for (let j = 0; j <= tValue; j++) {
              const Rn = A * (1 - j * i)
              if (Rn < -1e-10) {
                alert(`Балансовата стойност Rn става отрицателна (${Rn}) в година ${j}`)
                break
              }
              if (j > 0) {
                table.push({
